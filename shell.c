...
//Comando CD: Altera o diretório de trabalho atual do shell.
...
//Comando PWD: Imprime o caminho completo do diretório de trabalho atual.
...
//Comando LS: Lista os arquivos e diretórios em um caminho
...
//Comando RM: Remove (deleta) um arquivo
...
//Comando MV: Move ou renomeia um arquivo ou diretório
//Se a origem e destino estiverem no mesmo diretório
//ele renomeia. Se estiverem em diretórios diferentes
//ele move
...
//Comando RMDIR: Remove um diretório vazio
...
//Comando CAT: Concatena arquivos ou exibe o conteúdo de arquivos na saída padrão.
...
//Comando CP: Copia o conteúdo de um arquivo de origem para um arquivo de destino
void fazer_cp(char *args[]) {
    ...
    char caminho_destino_final[1024]; // Buffer para o novo caminho se for diretório
    char *nome_destino = args[2];     // Inicialmente, usa o argumento de destino

    // 1. Tentar obter informações do destino
    // Se o stat for bem-sucedido (o destino existe):
    if (stat(args[2], &info_destino) == 0) {
        
        // 2. Verificar se o destino existe e é um diretório
        if (S_ISDIR(info_destino.st_mode)) {
            
            // 3. Extrair o nome base do arquivo de origem (ignorando o caminho)
            char *nome_arquivo_origem = strrchr(args[1], '/');
            if (nome_arquivo_origem == NULL) {
                // Se não há barra, o nome do arquivo é o argumento inteiro
                nome_arquivo_origem = args[1];
            } else {
                // Se há barra, avança um caractere para ignorar a barra final
                nome_arquivo_origem++;
            }

            // 4. Construir o caminho completo
            size_t len_dir = strlen(args[2]);
            if (len_dir > 0 && args[2][len_dir - 1] != '/') {
                // Se o diretório não termina com /, adiciona-se
                snprintf(caminho_destino_final, sizeof(caminho_destino_final), "%s/%s", args[2], nome_arquivo_origem);
            } else {
                // Se já termina com /, concatena diretamente
                snprintf(caminho_destino_final, sizeof(caminho_destino_final), "%s%s", args[2], nome_arquivo_origem);
            }
            
            // 5. Atualiza o ponteiro de destino para o caminho completo construído
            nome_destino = caminho_destino_final;
        }
    }
    ...
    fd_destino = open(nome_destino, O_WRONLY | O_CREAT | O_TRUNC, 0666);
    if (fd_destino == -1) {
        perror("cp: destino");
        close(fd_origem);
        return;
...
//Comando SORT: Ordena as linhas de um arquivo de texto em ordem alfabética     
...
//Comando GREP: Executa a versão do sistema operacional do programa grep para buscar padrões em arquivos
